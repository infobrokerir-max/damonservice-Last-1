...
export default function MapOverview() {
  ...
  // Filter: Must have coordinates AND be approved
    ...
    <div className="h-[calc(100vh-140px)] rounded-xl overflow-hidden border border-gray-200 shadow-sm relative z-0">
          ...
          <Marker key={p.id} position={[p.tehran_lat!, p.tehran_lng!]}>
            <Popup>
              <div className="text-right font-vazir" dir="rtl">
                ...
                <div className="mt-2 text-xs px-2 py-1 rounded inline-block bg-green-100 text-green-700">
                  تایید شده
                </div>
              </div>
            </Popup>
      ...
      <div className="absolute top-4 right-4 z-[1000] bg-white p-3 rounded-lg shadow-md max-w-xs">
        ...
        <div className="text-xs space-y-1 text-gray-600">
          <p>تعداد پروژه‌های تایید شده: {validProjects.length}</p>
          <p>فقط پروژه‌هایی که وضعیت آن‌ها "تایید شده" است روی نقشه نمایش داده می‌شوند.</p>
        </div>
      </div>
    </div>
  );
