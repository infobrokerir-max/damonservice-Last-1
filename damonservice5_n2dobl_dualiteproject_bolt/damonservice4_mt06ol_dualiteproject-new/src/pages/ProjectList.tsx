...
import { Plus, Search, MapPin, Building2, UserCircle, Calendar, Loader2, X, AlertCircle } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          <Link to={`/projects/${project.id}`} key={project.id} className="block group">
            <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-all hover:border-sky-300 h-full flex flex-col relative overflow-hidden">
              
              {/* Status Indicator for New Inquiries */}
              {project.inquiry_status === 'pending' && (
                <div className="absolute top-0 left-0 bg-red-500 text-white text-[10px] px-2 py-1 rounded-br-lg z-10 font-bold flex items-center gap-1">
                  <AlertCircle size={10} />
                  استعلام جدید
                </div>
              )}
